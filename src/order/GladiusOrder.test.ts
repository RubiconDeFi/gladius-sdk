...
import { GladiusOrder, GladiusOrderInfo } from "./GladiusOrder";

describe("GladiusOrder", () => {
  const getOrderInfo = (data: Partial<GladiusOrderInfo>): GladiusOrderInfo => {
  ...
  it("parses a serialized order", () => {
    ...
    const order = new GladiusOrder(orderInfo, 1);
    const serialized = order.serialize();
    const parsed = GladiusOrder.parse(serialized, 1);
    expect(parsed.info).toEqual(orderInfo);
  });

  it("valid signature over info", async () => {
    const order = new GladiusOrder(getOrderInfo({}), 1);
